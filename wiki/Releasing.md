...
One workflow builds mlab-scw for macOS and Linux and puts everything on the
...
assets. It is
[`.github/workflows/release.yml`](https://github.com/mlab-sh/mlab-scw/blob/main/.github/workflows/release.yml),
...
`cargo test --locked` run first. Nothing is built for release if any of the
three fails.

That gate leaves one test out, deliberately: `corpus_answers` reaches
vuln.mlab.sh and is marked `#[ignore]`, so a release never depends on a third
party being up. Run it by hand when the advisory path changes:
...
`Formula/mlab-scw.rb`, with the four fresh checksums. It is generated by the
release job and does not exist until the first one has run — which is why there
is no formula in the repository yet and no hand-written one to go stale. It
